...
DECLARE
  ...
  paused integer := 0;
BEGIN
  -- Ativar robôs que estão no horário mas pausados
  UPDATE robots SET status = 'active', updated_at = now()
  WHERE status = 'paused'
    AND id IN (
  ...
  -- Pausar robôs que estão fora do horário mas ativos (apenas se tem escala configurada)
