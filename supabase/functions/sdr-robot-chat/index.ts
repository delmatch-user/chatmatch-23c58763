...
serve(async (req) => {
  ...
  try {
    ...
    systemPrompt += `- **REGRA CRÍTICA**: Responda com base nas informações presentes na sua Base de Conhecimento (Instruções, Perguntas e Respostas, Links de Referência e Documentos). Para dados públicos amplamente conhecidos (população de cidades, dados do IBGE, estatísticas demográficas, cálculos matemáticos, geografia), você PODE e DEVE usar seu conhecimento geral para enriquecer simulações, cálculos e respostas ao cliente. NUNCA invente ou alucine informações sobre o PRODUTO ou SERVIÇO que não estejam na sua base de conhecimento. Se a pergunta sobre o produto/serviço não puder ser respondida, informe educadamente e ofereça transferir para um atendente.\n`;
